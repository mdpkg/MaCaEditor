...
export function AiDiagramEditDialog({ format, path, currentSource, onApply, onClose, onOpenAiSettings }: Props) {
  ...
  return <div className="about-dialog-backdrop" onPointerDown={() => void close()}>
    <section className="about-dialog ai-result-dialog ai-diagram-dialog ai-diagram-edit-dialog" role="dialog" aria-modal="true" aria-label="AI Generate Diagram" onPointerDown={(e) => e.stopPropagation()}>
      <h2>AI Generate {format === "plantuml" ? "PlantUML" : "Mermaid"}</h2>
      <label className="ai-diagram-edit-instruction">Diagram description<textarea aria-label="Diagram generation description" value={instruction} disabled={stream.status === "running"} onChange={(e) => setInstruction(e.target.value)} /></label>
