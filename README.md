...
**File** → **Add Markdown...**および**Add Folder...**で階層を追加できます。ファイルツリーのコンテキストメニューではファイルとフォルダのrename、移動、削除、およびMarkdownのentrypoint指定を行えます。renameまたは移動では既存Markdown内のパッケージ内リンクが自動更新されます。削除されたファイルへのリンクは自動削除されません。空フォルダは保存されません。
